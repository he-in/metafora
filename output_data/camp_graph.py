...
def plotme(out_dir, data, name, offset=0, legend_loc=4, naieve_model=True):
    ...
    if naieve_model:
        ...
        out_dir, name, lerr.errors["MASE7"], lerr.errors["MASE7-sloped"], lerr.errors["MASE7-ratio"],
        lerr.errors["MASE30"], lerr.errors["MASE30-sloped"], lerr.errors["MASE30-ratio"], lerr.errors["N"]))
...
if __name__ == "__main__":
    ...
    if "refugee_debt" in refugee_data.columns:
        refugee_data.loc[:, ["total refugees (simulation)", "refugees in camps (simulation)", "raw UNHCR refugee count",
    ...
    if nmodel:
        print("%s & %s & %s & %s & %s & %s & %s\\\\" % (
        out_dir, sim_errors.get_error("MASE7"), sim_errors.get_error("MASE7-sloped"),
        sim_errors.get_error("MASE7-ratio"), sim_errors.get_error("MASE30"), sim_errors.get_error("MASE30-sloped"),
    ...
    print(out_dir, ": Averaged error normal: ", np.mean(diffdata), ", rescaled: ", np.mean(diffdata_rescaled),
          ", len: ", len(diffdata))
